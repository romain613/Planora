...
<div>
  ...
  {/* ── Modal modifier colonne (stage custom) ── */}
  {(typeof editingStage!=='undefined'?editingStage:null) && (
  ...
  {/* ── Modal confirmer suppression colonne ── */}
  {(typeof confirmDeleteStage!=='undefined'?confirmDeleteStage:null) && (
      ...
      return (<>
        ...
        {ct._linked && (
          <div style={{display:"flex",gap:8,marginBottom:20,paddingBottom:16,borderBottom:`1px solid ${T.border}`,flexWrap:"wrap",alignItems:"center"}}>
            ...
            {/* V1.12.x.2 — message confirmation enrichi si RDV confirmés futurs liés */}
                ...
                return(
                  <div key={b.id} style={{padding:'10px 12px',borderRadius:10,marginBottom:6,background:isUpcoming?stColor+'06':T.bg,border:`1px solid ${isUpcoming?stColor+'25':T.border}`,borderLeft:`4px solid ${stColor}`}}>
                    ...
                    <div style={{display:'flex',gap:4,marginTop:6}}>
                      ...
                      {isUpcoming&&b.status==='confirmed'&&<span onClick={()=>{setPhoneScheduleForm({contactId:ct.id,contactName:ct.name,number:ct.phone||'',date:b.date,time:b.time,duration:b.duration||30,title:b.title||'',notes:b.notes||'',calendarId:b.calendarId||(calendars||[])[0]?.id||'',_bookingMode:true,_editBookingId:b.id});setPhoneShowScheduleModal(true);}} style={{fontSize:10,color:'#F59E0B',cursor:'pointer',fontWeight:600,display:'flex',alignItems:'center',gap:3}}><I n="calendar" s={10}/> Replanifier</span>}
                      <span onClick={()=>{if(!confirm('Supprimer ce RDV ?'))return;api(`/api/bookings/${b.id}`,{method:'DELETE'}).then(r=>{if(r?.success!==false){/* V1.8.24.4 — DELETE retourne {booking, contact} actualisés : sync state */ if(r?.booking?.id){setBookings(p=>p.map(x=>x.id===r.booking.id?r.booking:x));}else{setBookings(p=>p.filter(x=>x.id!==b.id));}if(r?.contact?.id&&typeof setContacts==='function')setContacts(p=>p.map(c=>c.id===r.contact.id?r.contact:c));showNotif('RDV supprimé','success');}else showNotif(r?.error||'Erreur','danger');}).catch(()=>showNotif('Erreur','danger'));}} style={{fontSize:10,color:'#EF4444',cursor:'pointer',fontWeight:600,display:'flex',alignItems:'center',gap:3,marginLeft:'auto'}}><I n="trash-2" s={10}/> Supprimer</span>
                    </div>
                  </div>
                );
              };
        ...
        {collabFicheTab==="sms"&&(
          <div>
            {ct.phone ? (()=>{
              ...
              // Filter tolerant : backend normalise les phones (E.164) tandis que le front
              // garde le format contact — on compare sur les 9 derniers chiffres.
              const _last9 = (p) => String(p||'').replace(/\D/g,'').slice(-9);
              const _phoneLast9 = _last9(phone);
              const smsForContact = (_T.allSmsMessages||[]).filter(m => _phoneLast9 && (_last9(m.toNumber) === _phoneLast9 || _last9(m.fromNumber) === _phoneLast9));
              return <>
                {/* Compose SMS */}
                {(()=>{
