    ...
    <>
      {ct._linked && (
        <div style={{display:"flex",gap:8,marginBottom:20,paddingBottom:16,borderBottom:`1px solid ${T.border}`,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {ct.phone&&<Btn small disabled={isArchived} onClick={()=>{if(isArchived)return;setCollabFicheTab('sms');}} style={{color:'#0EA5E9',borderColor:'#0EA5E930',...lockedStyle}}><I n="message-square" s={13}/> SMS</Btn>}
          <Btn small disabled={isArchived} onClick={()=>{if(isArchived)return;setPhoneScheduleForm({contactId:ct.id,contactName:ct.name,number:ct.phone||'',date:new Date(Date.now()+86400000).toISOString().split('T')[0],time:'10:00',duration:30,notes:'',calendarId:(calendars||[])[0]?.id||'',_bookingMode:true});setPhoneShowScheduleModal(true);}} style={{color:'#0EA5E9',borderColor:'#0EA5E930',...lockedStyle}}><I n="calendar-plus" s={13}/> RDV</Btn>
          <Btn small disabled={isArchived} onClick={()=>{if(isArchived)return;handleCollabDeleteContact(ct.id);}} style={{color:'#64748B',borderColor:'#64748B30',...lockedStyle}}><I n="archive" s={13}/> Classer Perdu</Btn>
