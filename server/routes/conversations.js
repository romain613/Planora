...
router.get('/sms-history/:phone', requireAuth, (req, res) => {
  try {
    ...
    let messages;
    // Legacy safety: toNumber/fromNumber ont pu être stockés avec des espaces/séparateurs
    // (send pré-cleanPhone). On strip à la lecture via REPLACE() imbriqué pour que le
    // LIKE '%' || last9 matche quel que soit le format de stockage.
    const _normExpr = (col) =>
      `REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(${col},''),' ',''),'-',''),'.',''),'(', ''),')','')`;
    if (req.auth.role === 'admin' || req.auth.isSupra) {
      messages = db.prepare(`
        SELECT id, direction, content, status, createdAt, fromNumber, toNumber, collaboratorId, provider
        FROM sms_messages
        WHERE companyId = ? AND (${_normExpr('toNumber')} LIKE ? OR ${_normExpr('fromNumber')} LIKE ?)
      ...
      messages = db.prepare(`
        SELECT id, direction, content, status, createdAt, fromNumber, toNumber, collaboratorId, provider
        FROM sms_messages
        WHERE companyId = ? AND collaboratorId = ? AND (${_normExpr('toNumber')} LIKE ? OR ${_normExpr('fromNumber')} LIKE ?)
        ORDER BY createdAt DESC LIMIT 50
  ...
  try {
    ...
    sendPromise.then(result => {
        ...
        db.prepare(`INSERT INTO sms_messages (id, companyId, collaboratorId, contactId, direction, fromNumber, toNumber, content, status, conversationId, provider, twilioMessageSid, createdAt)
          ...
          .run(smsId, conv.companyId, req.auth.collaboratorId, conv.contactId, usedFrom, conv.clientPhone, content.trim(), conv.id, provider, result?.messageSid || null, new Date().toISOString());
