...
// V1.10.5 P4 — Option B : enrichissement batch des contacts déjà dispatchés d'une source.
// POST /api/leads/sources/:id/enrich-existing-contacts
// Boucle sur tous les leads dispatched de cette source, match contact via lead.contact_id,
// merge data_json → custom_fields_json sans écraser. Idempotent.
router.post('/sources/:id/enrich-existing-contacts', requireAdmin, enforceCompany, requirePermission('leads.manage'), (req, res) => {
  try {
    const sourceId = req.params.id;
    if (!verifyOwnership('lead_sources', sourceId, req, res)) return;

    // Récupère tous les leads dispatched de cette source qui ont un contact_id
    const leads = db.prepare(
      `SELECT id, contact_id, data_json FROM incoming_leads
       WHERE source_id = ? AND companyId = ?
         ...
         AND dispatched = 1`
    ).all(sourceId, req.auth.companyId);
    ...
    for (const lead of leads) {
      ...
      } catch (e) { errors++; console.error('[ENRICH BATCH]', e.message); }
    }
...
// V1.10.6 — preview suppression enveloppe : compte unassigned vs assigned (contact_id rempli).
// Permet au frontend de décider entre confirm direct ou modale "désassigner d'abord".
router.get('/envelopes/:id/delete-preview', requireAdmin, requirePermission('leads.view'), (req, res) => {
  try {
    if (!verifyOwnership('lead_envelopes', req.params.id, req, res)) return;
    const env = db.prepare('SELECT id, name, companyId FROM lead_envelopes WHERE id = ?').get(req.params.id);
    if (!env) return res.status(404).json({ error: 'envelope_not_found' });

    const total = db.prepare('SELECT COUNT(*) AS n FROM incoming_leads WHERE envelope_id = ? AND companyId = ?').get(env.id, env.companyId).n;
    const assigned = db.prepare("SELECT COUNT(*) AS n FROM incoming_leads WHERE envelope_id = ? AND companyId = ? AND contact_id IS NOT NULL AND contact_id != ''").get(env.id, env.companyId).n;
    const unassigned = total - assigned;

    // Échantillon (top 5) des leads assignés pour affichage UI
    const samples = db.prepare(`
      ...
      WHERE il.envelope_id = ? AND il.companyId = ?
        AND il.contact_id IS NOT NULL AND il.contact_id != ''
      ORDER BY il.created_at ASC
      LIMIT 5
    `).all(env.id, env.companyId).map(r => ({
      leadId: r.leadId,
    ...
    res.json({ envelope: { id: env.id, name: env.name }, total, unassigned, assigned, assignedSamples: samples });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// V1.10.6 — DELETE envelope avec 2 modes :
//   - default (cascade=false) : comportement legacy (envelope+rules supprimés, leads.envelope_id=NULL conservés)
//   - cascade=true            : hard delete leads + envelope + rules en transaction atomique
//                               * si leads assignés présents ET force!=true → 409 leads_assigned
//                               * si force=true → désassigne d'abord (sans toucher contacts CRM) puis hard delete
// Désassignation = retire UNIQUEMENT le lien lead → contact (incoming_leads.contact_id, status, assigned_to)
// + DELETE lead_assignments. NE TOUCHE PAS contacts.* (pipeline/status/RDV/notes préservés).
router.delete('/envelopes/:id', requireAdmin, requirePermission('leads.manage'), (req, res) => {
  try {
    ...
    if (!cascade) {
      ...
      db.prepare('DELETE FROM lead_dispatch_rules WHERE envelope_id = ?').run(env.id);
      db.prepare("UPDATE incoming_leads SET envelope_id = NULL WHERE envelope_id = ?").run(env.id);
      remove('lead_envelopes', env.id);
      logHistory(env.companyId, 'envelope_deleted', { envelope_id: env.id, name: env.name, mode: 'legacy_keep_leads' }, { user_id: req.auth?.userId, user_name: req.auth?.name });
      return res.json({ success: true, mode: 'legacy', deletedLeads: 0 });
    }

    // cascade=true
    const totalLeads = db.prepare('SELECT COUNT(*) AS n FROM incoming_leads WHERE envelope_id = ? AND companyId = ?').get(env.id, env.companyId).n;
    const assignedCount = db.prepare("SELECT COUNT(*) AS n FROM incoming_leads WHERE envelope_id = ? AND companyId = ? AND contact_id IS NOT NULL AND contact_id != ''").get(env.id, env.companyId).n;
    ...
    // Transaction atomique : désassign (si force) → DELETE leads → DELETE rules → DELETE envelope
      ...
      if (assignedCount > 0 && force) {
        ...
        const r = db.prepare(`
          ...
          WHERE envelope_id = ? AND companyId = ?
            AND contact_id IS NOT NULL AND contact_id != ''
        `).run(env.id, env.companyId);
        unassignedDuringTx = r.changes || 0;
      }
      // DELETE lead_assignments des leads de l'enveloppe
      const ra = db.prepare(`
        DELETE FROM lead_assignments
        WHERE lead_id IN (SELECT id FROM incoming_leads WHERE envelope_id = ? AND companyId = ?)
          AND companyId = ?
      `).run(env.id, env.companyId, env.companyId);
      deletedAssignmentsCount = ra.changes || 0;
      // HARD DELETE leads
      const rl = db.prepare('DELETE FROM incoming_leads WHERE envelope_id = ? AND companyId = ?').run(env.id, env.companyId);
      deletedLeadsCount = rl.changes || 0;
      // DELETE dispatch rules
      db.prepare('DELETE FROM lead_dispatch_rules WHERE envelope_id = ?').run(env.id);
      // DELETE envelope
      db.prepare('DELETE FROM lead_envelopes WHERE id = ? AND companyId = ?').run(env.id, env.companyId);
    });
    tx();

    logHistory(env.companyId, 'envelope_deleted', {
      envelope_id: env.id, name: env.name, mode: 'cascade',
      totalLeads, assignedDetected: assignedCount, unassignedDuringTx,
      deletedLeads: deletedLeadsCount, deletedAssignments: deletedAssignmentsCount,
      force
    }, { user_id: req.auth?.userId, user_name: req.auth?.name });

    res.json({
      success: true, mode: 'cascade',
...
router.post('/import/gsheet-preview', requireAdmin, requirePermission('leads.manage'), async (req, res) => {
  try {
    ...
    // Auto-detect mapping for suggestion (legacy simple format pour backward compat)
    const suggestedMapping = autoDetectMapping(parsed.headers);
    // V1.10.5 P3 — output enrichi avec type + confidence + label (pour modal mapping V2)
    const sampleRowsForDetect = parsed.rows.slice(0, 5);
    const suggestedMappingDetailed = autoDetectMappingV2(parsed.headers, sampleRowsForDetect);

    res.json({ success: true, headers: parsed.headers, sampleRows: sampleRowsForDetect, totalRows: parsed.rows.length, sheetId, gid, suggestedMapping, suggestedMappingDetailed });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
function assignLeadToCollab(lead, collabId, ruleId, companyId, now, envelope_id, userId, userName) {
  ...
  if (existingContact) {
    ...
    // V1.10.5 P4 — enrichissement Option A (inline) : merge data_json → custom_fields_json
    // sans écraser les valeurs existantes non-vides. Audit log auto, idempotent.
    try {
      enrichContactCustomFields(contactId, lead.data_json, null);
    } catch (e) { console.error('[ENRICH INLINE]', e.message); }
  } else {
    ...
    // V1.10.5 P2 — propage les colonnes Sheet non-standards vers contacts.custom_fields_json
    const _customFields = extractCustomFieldsArray(extraData);
    insert('contacts', {
      ...
      custom_fields_json: JSON.stringify(_customFields),
      createdAt: now
    });
  }
