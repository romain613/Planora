...
router.post('/contacts/check-duplicates', requireAuth, enforceCompany, (req, res) => {
  try {
    ...
    if (emails && emails.length) {
      ...
      const rows = db.prepare(`SELECT LOWER(email) as em FROM contacts WHERE companyId = ? AND LOWER(email) IN (${placeholders}) AND email != ''`).all(companyId, ...emails.map(e => e.toLowerCase().trim()));
      ...
      try {
        ...
        const tx = db.transaction(() => {
          ...
          insert('pipeline_history', {
            id: phId, contactId: id, companyId: authCompanyId,
