...
export function computeNextActions(collaboratorId, companyId) {
  ...
  try {
    const nrpContacts = db.prepare(
      "SELECT id, name, phone, email, nrp_next_relance, lead_score FROM contacts WHERE companyId = ? AND assignedTo = ? AND pipeline_stage = 'nrp' AND nrp_next_relance != '' AND nrp_next_relance <= ? ORDER BY nrp_next_relance ASC LIMIT 5"
  ...
  try {
    ...
    const rdvContacts = db.prepare(
      "SELECT id, name, phone, email, next_rdv_date, lead_score FROM contacts WHERE companyId = ? AND assignedTo = ? AND pipeline_stage = 'rdv_programme' AND next_rdv_date != '' AND next_rdv_date < ? ORDER BY next_rdv_date ASC LIMIT 5"
  ...
  try {
    ...
    const analyses = db.prepare(
      "SELECT a.contactId, a.actionItems_json, a.followupType, a.followupDate, a.summary, c.name, c.phone, c.email, c.lead_score FROM ai_copilot_analyses a JOIN contacts c ON c.id = a.contactId WHERE a.companyId = ? AND a.collaboratorId = ? AND a.createdAt > ? AND a.followupType IS NOT NULL AND a.followupType != '' ORDER BY a.createdAt DESC LIMIT 5"
  ...
  try {
    ...
    const qualContacts = db.prepare(
      "SELECT id, name, phone, email, updatedAt, lead_score FROM contacts WHERE companyId = ? AND assignedTo = ? AND pipeline_stage = 'qualifie' AND updatedAt < ? AND updatedAt != '' ORDER BY lead_score DESC LIMIT 5"
  ...
  try {
    const devisContacts = db.prepare(
      "SELECT id, name, phone, email, contract_amount, lead_score, updatedAt FROM contacts WHERE companyId = ? AND assignedTo = ? AND contract_amount > 0 AND contract_signed = 0 AND pipeline_stage NOT IN ('perdu', 'client_valide') ORDER BY contract_amount DESC LIMIT 5"
    ).all(companyId, collaboratorId);
  ...
  try {
    ...
    const inactifs = db.prepare(
      "SELECT id, name, phone, email, updatedAt, lead_score FROM contacts WHERE companyId = ? AND assignedTo = ? AND pipeline_stage IN ('contacte', 'qualifie') AND updatedAt < ? AND updatedAt != '' ORDER BY lead_score DESC LIMIT 5"
    ).all(companyId, collaboratorId, sevenDaysAgo);
