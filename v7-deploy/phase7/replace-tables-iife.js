...
const START = '{portalTab === "tables" && (()=>{';
const REPLACEMENT = '        {portalTab === "tables" && <TablesTab/>}';
...
console.log(`Tables IIFE: lines ${startIdx + 1} → ${endIdx + 1} (${endIdx - startIdx + 1} lines)`);

const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
fs.writeFileSync(`${PATH}.pre-tables-${ts}`, src);
