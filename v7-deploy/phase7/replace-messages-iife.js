...
const START = '{portalTab === "messages" && (()=>{';
const REPLACEMENT = '        {portalTab === "messages" && <MessagesTab/>}';
...
console.log(`Messages IIFE: lines ${startIdx + 1} → ${endIdx + 1} (${endIdx - startIdx + 1} lines)`);

const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
fs.writeFileSync(`${PATH}.pre-messages-${ts}`, src);
