#!/usr/bin/env node
// Phase 14b — extract Phone/Pipeline Live tab from CollabPortal.jsx into features/collab/tabs/PhoneTab.jsx
// THE MASTODON: ~6 791 lines of VoIP + dialer + conversations + history + AI copilot live.
...
const START_PATTERN = '{portalTab === "phone"';
let startIdx = -1;
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes(START_PATTERN) && lines[i].includes('voipConfigured')) {
    startIdx = i;
    break;
  }
}
...
// End: find the matching `        )}` at column 8 with paren depth 0
  ...
  for (let i = idx; i < lines.length; i++) {
    for (const c of lines[i]) {
      if (c === '(') depth++;
        ...
        depth--;
        if (depth === 0 && lines[i].trim().endsWith(')}')) return i;
      }
    }
  }
  return -1;
...
const endIdx = findBlockEnd(startIdx);
...
// Body = the JSX between `{cond && (` and `)}` — strip those wrappers
// The first line is `        {portalTab === "phone" && (typeof voipConfigured...) && (`
// The last line is `        )}` (or similar)
// Body = lines[startIdx+1..endIdx-1]
...
const PhoneTab = () => {
  // Destructure ALL refs Phone tab needs (~150 refs from context)
...
fs.writeFileSync(`${PATH}.pre-phone-${ts}`, src);
