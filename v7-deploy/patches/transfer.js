...
router.put('/executor/:contactId', requireAuth, enforceCompany, (req, res) => {
  try {
    ...
    const companyId = req.auth.companyId;
    const sourceCollabId = req.auth.collaboratorId;
    ...
    // Verify target collaborator exists in same company
    const targetCollab = db.prepare('SELECT id, name FROM collaborators WHERE id = ? AND companyId = ?').get(executorCollabId, companyId);
